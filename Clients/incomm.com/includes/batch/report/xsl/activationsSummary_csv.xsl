...
<xsl:template match="/">
	...
	<xsl:variable name="activations" select="/root/activations" />
	<xsl:variable name="deactivations" select="/root/deactivations" />
		...
		<xsl:for-each select="exsl:node-set($countries)/country">
			...
			<xsl:for-each select="exsl:node-set($upcs)/upc">
				...
				<xsl:for-each select="exsl:node-set($margins)/margin">
					...
					<xsl:variable name="activationAllRows" select="exsl:node-set($activations)//gift[
          ...
          reservation/inventory/activationMargin=$margin
					...
					<xsl:if test="count($activationAllRows) > 0">
					<xsl:call-template name="stdrow">
						<xsl:with-param name="row" select="$activationAllRows[1]" />		
						<xsl:with-param name="allRows" select="exsl:node-set($activationAllRows)" />		
					</xsl:call-template>
					</xsl:if>
					
					<!-- Deactivations -->
					<xsl:variable name="deactivationAllRows" select="exsl:node-set($deactivations)//gift[
					partner=$selectedPartner and
          reservation/inventory/locationId=$location and
          messages/message/shoppingCart/transaction/country=$country and
          product/upc=$upc and
          reservation/inventory/activationMargin=$margin
					...
					<!-- create the actual row -->
					<xsl:if test="count($deactivationAllRows) > 0">
					<xsl:call-template name="stdrow">
						<xsl:with-param name="row" select="$deactivationAllRows[1]" />		
						<xsl:with-param name="allRows" select="exsl:node-set($deactivationAllRows)" />		
					</xsl:call-template>
					</xsl:if>
          ...
          messages/message[1]/gift/reservation/inventory/activationMargin=$margin
					...
					<xsl:call-template name="chbkrow">
						<xsl:with-param name="row" select="$chargebackAllRows" />		
  ...
  <xsl:text>,</xsl:text>
	...
	<xsl:variable name="discountAmount">
  	<xsl:choose>
  		...
  		<xsl:when test="$row/../type='ndr'">0</xsl:when>
			<xsl:otherwise><xsl:value-of select="0" /></xsl:otherwise>
  	</xsl:choose>
	</xsl:variable>

	<!-- Total Amount -->
	<xsl:value-of select="-1 * sum($allRows/../amount) + $discountAmount" />
	<xsl:text>,</xsl:text>
	...
	<xsl:variable name="fees">
  	<xsl:choose>
  		<xsl:when test="$row/../type='activation'">
  			<xsl:value-of select="count($allRows) * 
  			$row/../../../activationFees/ledger[gift/id=$row/../giftId]/amount" />
  		</xsl:when>
  		<xsl:when test="$row/../type='deactivation'">
  			<xsl:value-of select="count($allRows) * -1 * 
  			$row/../../../deactivationFees/ledger[gift/id=$row/../giftId]/amount" />
  		</xsl:when>
  		<xsl:when test="$row/../type='ndr'">
  			<xsl:value-of select="count($allRows) * 
  			$row/../../../ndrFees/ledger[gift/id=$row/../giftId]/amount" />
  		</xsl:when>
  	</xsl:choose>
	</xsl:variable>
