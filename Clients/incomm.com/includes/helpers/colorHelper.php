...
class colorHelper {
  ...
  public static function RGBToHTML($RGB)
  {
    $r = 0xFF & ($RGB >> 0x10);
    $g = 0xFF & ($RGB >> 0x8);
    $b = 0xFF & $RGB;
  ...
  public static function changeLuminosity( $HEX, $LuminosityPercent) {
    $RGB = self::HTMLToRGB( $HEX );
    $l = self::getLfromRGB($RGB);
    $HSL = self::RGBToHSL($RGB);


    $NewHSL = (int)(((float)(( $l * 100) + $LuminosityPercent ) / 100) * 255) + (0xFFFF00 & $HSL);
    return self::RGBToHTML( self::HSLToRGB($NewHSL));
  }

  public static function getLfromRGB( $rgb ) {
    $r = 0xFF & ($rgb >> 0x10);
    $g = 0xFF & ($rgb >> 0x8);
    $b = 0xFF & $rgb;
