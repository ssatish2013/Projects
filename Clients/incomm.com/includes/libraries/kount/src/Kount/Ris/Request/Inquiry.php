 ...
 * <code>
 * <?php
 *   include '/path/to/sdk/autoload.php';
 *
 *   try {
 *     $inquiry = new Kount_Ris_Request_Inquiry();
 *
 *     // session id (SESS) is a unique identifier for this customer's
 *     // interaction with your server. In php this this often the output of
 *     // session_id()
 ...
 *     // for a PayPal payment: $inquiry->setPaypalPayment("testingpaypal111111");
 *     // for a check payment: $inquiry->setCheckPayment("123456789");
 *     // for a Google payment: $inquiry->setGooglePayment("123456789012345");
 *     // for a BillMeLater payment: $inquiry->setBillMeLaterPayment("1234567890123456");
 *     // for no payment (for example, a trial sign up): $inquiry->setNoPayment();
 *
 ...
 * @version $Id: Inquiry.php 19013 2012-03-29 17:59:09Z mmn $
 * @copyright 2012 Kount, Inc. All Rights Reserved.
 */
class Kount_Ris_Request_Inquiry extends Kount_Ris_Request {

	const MAX_UDF_VALUE_LENGTH = 255;
  ...
  public function setUserDefinedField ($label, $value) {
    $index = "UDF[{$label}]";
    $this->data[$index] = substr($value, 0, self::MAX_UDF_VALUE_LENGTH);
    return $this;
  }
  ...
  public function setMode ($mode) {
    if ((self::MODE_Q != $mode) && (self::MODE_P != $mode)) {
      throw new Kount_Ris_IllegalArgumentException(
          "Invalid RIS inquiry mode [{$mode}]. Must be 'Q' or 'P'");
    }
    $this->data['MODE'] = $mode;
    return $this;
  }
